...
bool TrpcCodec::onDecodeRequestOrResponseProtocol(std::string&& header_raw) {
  ASSERT(metadata_.getMessageType() == MetaProtocolProxy::MessageType::Request ||
         metadata_.getMessageType() == MetaProtocolProxy::MessageType::Response);

  if (metadata_.getMessageType() == MetaProtocolProxy::MessageType::Request) {
